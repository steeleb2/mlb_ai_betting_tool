...
# This version includes live odds integration and value detection for Moneyline, Spreads, Over/Unders, and Player Props
...
PROPS_API_URL = "https://api.the-odds-api.com/v4/sports/baseball_mlb/odds?markets=player_props"
...
def fetch_player_props():
    ...
    if response.status_code == 200:
        ...
        st.error(f"Failed to fetch player props: {response.status_code}")
        return []
...
# Player Props
st.markdown("---")
st.write("### Live Player Prop Value Picks")
if not prop_data:
            ...
            for market in bm.get("markets", []):
                ...
                for outcome in market.get("outcomes", []):
                    ...
                    if odds is not None:
                        ...
                        value = model_prob - imp_prob
                        if value > 0.05:
                            st.write(f"- {player} ({market_type}) o{point} ({odds}) | Edge: +{round(value * 100, 1)}%")
