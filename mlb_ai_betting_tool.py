        ...
        for bm in data.get("bookmakers", []):
            for mkt in bm.get("markets", []):
                ...
                for outcome in mkt.get("outcomes", []):
                    ...
                    desc   = outcome["name"]  # e.g. "Over 1.5"
                    ao     = outcome["price"]
...
st.dataframe(
    top25[
        ["player", "prop_desc", "american_odds", "decimal_odds", "implied_prob"]
    ],
    height=600
)

st.subheader("Top 25 Players by Decimal Odds")
# chart by player name
st.bar_chart(top25.set_index("player")["decimal_odds"])
