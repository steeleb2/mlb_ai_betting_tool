...
# supported MLB player‐props market keys
MLB_PROPS_MARKETS = [
        ...
        for bm in data.get("bookmakers", []):
            for mkt in bm.get("markets", []):
                ...
                for outcome in mkt.get("outcomes", []):
                    # use participant field if available
                    ...
                    if participant:
                        ...
                        # fallback: parse "Name Over/Under X.Y"
                        m = re.match(r"^(.*?)\s+(Over|Under)\s+(.+)$", name)
                        if m:
                            player = m.group(1)
...
st.subheader("Implied Probability Chart")
